...
Example:
  python split_files_by_time.py \
    -- srcdir r'F:\Photos\Lightroom\2022\2022-12-11' \
    -- dstdir r'F:\Photos\Lightroom\2022\2022-12-11_split' \
    -- ext jpg \
...
if __name__ == '__main__':
  ...
  # args.srcdir = r'C:\Users\gezhe\OneDrive\Pictures\Lightroom Saved Photos\2022-12-11 Dallas Marathon Non-DASH'
  # args.dstdir = r'C:\Users\gezhe\OneDrive\Pictures\Lightroom Saved Photos\2022-12-11 Dallas Marathon Non-DASH_split'
  # args.ext = 'jpg' # photos extension
  # args.starttime = datetime.datetime(2022,12,11,0,0,0) # used to filter out photos taken before this time
  # args.itvlmin = 10 # interval min (current version upports from value 1 up to 60 min, i.e. 1 hour only)
  ...
  # get start time
  if args.starttime:
    ymd, hm = str(args.starttime).split()
    ymd = ymd.replace('-', '')
    hm = hm.replace(':','')[:4]
  ...
  for i, imgfile in enumerate(imgfiles):
    if i % itvl == 0:  
      print('extracting time from photos {} to {} ...'.format(i, i+itvl))  
  ...
  nimgfiles_filtered = len(file2time_filtered)  
  print('# of image files after filtering: {}'.format(nimgfiles_filtered))  
  ...
  # copy photo files to the destination sub dir
  for i, (basename, takentime) in enumerate(file_n_time_sorted):
    ...
    if os.path.isfile(outfile):
      print('{}/{}: {} exists, skip!'.format(i+1, nimgfiles_filtered, outfile))
    else:
      print('{}/{}: {} with {} --> {}'.format(i+1, nimgfiles_filtered, basename, takentime, basedir))
